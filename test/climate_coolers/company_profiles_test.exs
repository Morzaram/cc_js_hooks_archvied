defmodule ClimateCoolers.CompanyProfilesTest do
  ...
  describe "company_profiles" do
    ...
    @invalid_attrs %{description: nil, established: nil, name: nil, url: nil}
    ...
    test "create_profile/1 with valid data creates a profile" do
      valid_attrs = %{description: "some description", established: ~D[2022-02-06], name: "some name", url: "some url"}

      assert {:ok, %Profile{} = profile} = CompanyProfiles.create_profile(valid_attrs)
      assert profile.description == "some description"
      assert profile.established == ~D[2022-02-06]
      assert profile.name == "some name"
    ...
    test "update_profile/2 with valid data updates the profile" do
      profile = profile_fixture()
      update_attrs = %{description: "some updated description", established: ~D[2022-02-07], name: "some updated name", url: "some updated url"}

      assert {:ok, %Profile{} = profile} = CompanyProfiles.update_profile(profile, update_attrs)
      assert profile.description == "some updated description"
      assert profile.established == ~D[2022-02-07]
      assert profile.name == "some updated name"
